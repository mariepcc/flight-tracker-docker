    ...
    <nav className="fixed top-0 left-0 right-0 z-100 px-6 py-6">
      <div className="max-w-5xl mx-auto flex items-center justify-between glass-card px-6 py-3 border-white/5 bg-black/40 backdrop-blur-2xl">
        ...
        <div className="flex items-center gap-2">
          <Link href="/">
            <div className={`flex items-center gap-2 px-5 py-2.5 rounded-2xl text-xs font-black uppercase tracking-widest transition-all duration-300 ${
              pathname === '/' 
          ...
          <Link href="/wishlist">
            <div className={`flex items-center gap-2 px-5 py-2.5 rounded-2xl text-xs font-black uppercase tracking-widest transition-all duration-300 relative group/item ${
              pathname === '/wishlist' 
              ? 'bg-indigo-600 text-white shadow-[0_0_25px_rgba(79,70,229,0.4)]' 
              : 'bg-white/5 text-slate-500 hover:text-white hover:bg-white/10'
            }`}>
              <Heart className={`w-4 h-4 transition-transform group-hover/item:scale-110 ${
              ...
              <motion.span 
                initial={{ scale: 0 }} 
                animate={{ scale: 1 }} 
                className="absolute -top-1 -right-1 w-2.5 h-2.5 bg-rose-500 rounded-full border-2 border-[#020408]" 
              />
            </div>
          </Link>
        </div>
