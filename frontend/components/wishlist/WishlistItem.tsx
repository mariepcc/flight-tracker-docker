...
export default function WishlistItem({ city, currentPrice, lastUpdate, date, onShowDetails }: any) {
  return (
    <motion.div 
    ...
    >
      <div className="flex items-center gap-6">
        ...
        <div>
          ...
          <p className="text-sm text-slate-500">{date} • Updated {lastUpdate}</p>
        </div>
      </div>

      <div className="flex items-center gap-8">
        <div className="text-right">
          <p className="text-xs text-slate-500 uppercase tracking-widest font-bold">Latest Price</p>
          <p className="text-2xl font-black text-white">{currentPrice} PLN</p>
        </div>
        <Button 
